...
function sostavChisla(massivChisel, chislo) {
    ...
    let result = [];
    recursive(massivChisel, chislo, 0, [], result);
    return result;
}

function recursive(massivChisel, chislo, lastIndex, currentCombination, res) {
    if (chislo === 0) {
        res.push(currentCombination.slice());
        return;
    }
    if (chislo < 0 || lastIndex === massivChisel.length) {
        return;
    }

    for (let i = lastIndex; i < massivChisel.length; i++) {
        if (i > lastIndex && massivChisel[i] === massivChisel[i - 1]) {
            continue;
        }
        currentCombination.push(massivChisel[i]);
        recursive(
            massivChisel,
            chislo - massivChisel[i],
            i + 1,
            currentCombination,
            res
        );
